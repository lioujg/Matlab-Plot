...
bag_select = "2021-08-12-18-07-20.bag";
...
Y_sum = zeros(1,320);
for i = 1:320
    X_sum(1,i) = c2_X(2*i) - c2_desired_X(i);
    Y_sum(1,i) = c2_Y(2*i) - c2_desired_Y(i);
end
X_RMSE = sqrt(sum(X_sum.^2)/320);
Y_RMSE = sqrt(sum(Y_sum.^2)/320);
RMSE = sqrt((sum(X_sum.^2) + sum(Y_sum.^2))/320);
