...
bag_select = "2021-08-19-15-03-07.bag";

if bag_select == "2021-08-19-14-54-05.bag"
        length_of_t = 16184;
        pos_front_null = 1;
        pos_back_null = 970;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
        desired_pos_front_null = 270;
        desired_pos_back_null = 565;
        c2_desired_Z = ones(desired_pos_back_null-desired_pos_front_null-1 ,1) * 0.36;
elseif  bag_select == "2021-08-19-14-59-06.bag"
        length_of_t = 16184;
        pos_front_null = 1;
        pos_back_null = 970;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
        desired_pos_front_null = 205;
        desired_pos_back_null = 499;
        c2_desired_Z = ones(desired_pos_back_null-desired_pos_front_null-1 ,1) * 0.31;
elseif  bag_select == "2021-08-19-15-01-00.bag"
        length_of_t = 16184;
        pos_front_null = 1;
        pos_back_null = 970;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
        desired_pos_front_null = 213;
        desired_pos_back_null = 508;
        c2_desired_Z = ones(desired_pos_back_null-desired_pos_front_null-1 ,1) * 0.31;
elseif  bag_select == "2021-08-19-15-03-07.bag"
        length_of_t = 16184;
        pos_front_null = 1;
        pos_back_null = 970;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
        desired_pos_front_null = 359;
        desired_pos_back_null = 654;
        c2_desired_Z = ones(desired_pos_back_null-desired_pos_front_null-1 ,1) * 0.31;
end
...
%RMSE
% desire_size = size(t_c2_desired,2);
% X_sum = zeros(1,desire_size);
% Y_sum = zeros(1,desire_size);
% 
% polyfit_X = polyfit(t_c2,c2_X,8);
% polyfit_Y = polyfit(t_c2,c2_Y,8);
% X_fit = polyval(polyfit_X,t_c2_desired);
% Y_fit = polyval(polyfit_Y,t_c2_desired);
...
plot(t_c2, c2_X, 'r')
hold on
plot(t_c2_desired, c2_desired_X, 'b--')
hold on
plot(t_c2, X_sum, 'm-.')
y_label = ylabel('X (m)', 'rotation', 0);
set(y_label, 'Units', 'Normalized', 'Position', [-0.12, 0.43]);
xlim([0 t_c2(end)])
...
plot(t_c2, c2_Y, 'r')
hold on
plot(t_c2_desired, c2_desired_Y, 'b--')
hold on
plot(t_c2, Y_sum, 'm-.')
...
for k=1:numel(scatter_x)
      text(scatter_x(k),scatter_y(k)-0.05,['(' num2str(scatter_x(k)) ',' num2str(scatter_y(k)) ')'])
end
title("figure 9", 'Fontsize', 11);
