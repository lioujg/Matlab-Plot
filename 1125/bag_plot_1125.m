...
bag_select = "2021-11-25-19-08-40.bag";

if bag_select == "2021-11-25-18-56-50.bag"
        pos_front_null = 1;
        pos_back_null = 1489;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
elseif  bag_select == "2021-11-25-19-08-40.bag"
        pos_front_null = 1;
        pos_back_null = 1124;
        t_front_null = pos_front_null*2;
        t_back_null = pos_back_null*2;
...
payload_bag = select(bag, 'topic', '/robot_1/estimated/mass');
Robot_1_msgStructs = readMessages(payload_bag, 'DataFormat', 'struct');
Robot_1_mass = cellfun(@(m) double(m.X), Robot_1_msgStructs);

% obtain payload mass estimated by Robot 2
payload_bag = select(bag, 'topic', '/robot_2/estimated/mass');
Robot_2_msgStructs = readMessages(payload_bag, 'DataFormat', 'struct');
Robot_2_mass = cellfun(@(m) double(m.X), Robot_2_msgStructs);

% obtain payload inertia estimated by Robot 1
payload_bag = select(bag, 'topic', '/robot_1/estimated/inertia');
Robot_1_msgStructs = readMessages(payload_bag, 'DataFormat', 'struct');
Robot_1_inertia_Ixx = cellfun(@(m) double(m.X), Robot_1_msgStructs);
Robot_1_inertia_Iyy = cellfun(@(m) double(m.Y), Robot_1_msgStructs);
Robot_1_inertia_Izz = cellfun(@(m) double(m.Z), Robot_1_msgStructs);
